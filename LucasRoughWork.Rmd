...
# -----------------------------------------
# CH2O
m = lm(BMI ~ ns(CH2O, df = 5), data = trainData)
p = predict(m, newdata = testData)
r2 = sqrt(mean((BMI - p)^2))
# -----------------------------------------
...
library(glmnet)
# Create model
fit = lm(BMI ~ Gender + NCP + SMOKE + CH2O + TUE, data = trainData)
X = model.matrix(fit)
y = BMI
...
# Set up x test
X_test = model.matrix(~Gender + NCP + SMOKE + CH2O + TUE, data = testData)
